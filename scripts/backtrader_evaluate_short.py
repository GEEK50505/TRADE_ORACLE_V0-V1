"""
Chronological Backtrader validation for the TRADE_ORACLE short-only shortlist.

This evaluator mirrors the structure of `backtrader_evaluate.py`, but it is
specialized for short-side research so the short phase can stay fully separate
from the validated long pipeline.

Recommended full run:
    python scripts/backtrader_evaluate_short.py

Helpful smoke test:
    python scripts/backtrader_evaluate_short.py --limit 2 --output-prefix results/backtrader_short_smoke
"""
...
DEFAULT_CANDIDATES_PATH = os.path.join(RESULTS_DIR, "short_candidates.csv")
...
DEFAULT_OUTPUT_PREFIX = os.path.join(RESULTS_DIR, "backtrader_short")
...
class CandidateConfig:
    """A typed container for one row from the short candidate CSV."""
    ...
    def from_row(cls, row: pd.Series) -> "CandidateConfig":
        """
        Build a stable short-side candidate object from a pandas row.

        The loader remains flexible enough to support future short-side 4H
        confirmation experiments, even though the initial short sweep is Daily-only.
        """
        ...
        side = str(first_present(row, ["side"], "short")).lower()
        param_id = first_present(row, ["param_id"], None)
        if param_id is None:
            param_id = f"short_cand_{rank:03d}_{source_symbol.replace('/', '_')}"
        ...
        confirm_mode = str(
            first_present(
                row,
                ["confirm_mode"],
                "close_below_ema" if confirm_enabled else "disabled",
            )
        )
...
class MavenShortCandidateStrategy(bt.Strategy):
    """
    One complete chronological evaluation of one short candidate parameter set.
    ...
    def _portfolio_equity(self, mark: str = "close") -> float:
        """
        Compute portfolio equity with short-side unrealized PnL.
        """
        ...
        for state in self.asset_states.values():
            ...
            equity += (float(position["entry_price"]) - current_price) * float(position["remaining_size"])
        return float(equity)

    def _update_drawdown_state(self) -> None:
        """
        Update the short-side account watermark and trailing-drawdown state.

        For shorts:
        1. Mark open positions to the bar low to allow the equity watermark to improve.
        2. Mark open positions to the bar high to measure the worst adverse excursion.
        """

        low_equity = self._portfolio_equity(mark="low")
        high_equity = self._portfolio_equity(mark="high")

        if low_equity > self.peak_equity:
            self.peak_equity = low_equity

        drawdown_dollars = max(0.0, self.peak_equity - high_equity)
    ...
    def _confirmation_is_valid(self, state: Dict[str, object]) -> bool:
        """
        Check the optional short-side execution confirmation filter.

        Current confirmation logic is the short mirror of the long filter:
        - execution close must be below the execution EMA
        - the distance below EMA must meet the configured minimum threshold
        """
    ...
    def _signal_is_valid(self, state: Dict[str, object]) -> Optional[Dict[str, float]]:
        """
        Mirror the short-side discovery confluence logic exactly.

        Current short discovery logic is:
        - price below SMA
        - bounce retracement inside the configured fib zone
        - close near EMA within the configured buffer
        - downside momentum threshold satisfied over the configured lookback
        """
        ...
        is_valid = (
            close < sma
            and self.candidate.fib_floor <= bounce_retracement <= self.candidate.fib_band
            and ema_distance <= self.candidate.eb
            and downside_momentum >= self.candidate.rw
        )
        ...
        return {
            "atr": atr,
            "signal_close": close,
            "bounce_retracement": bounce_retracement,
            "downside_momentum": downside_momentum,
        }

    def _schedule_entry_if_needed(self, state: Dict[str, object]) -> None:
        """
        Create a pending short entry when a new daily signal appears.
    ...
    def _try_open_pending_entry(self, state: Dict[str, object]) -> None:
        """
        Open a pending short position at the next execution-bar open if portfolio
        rules allow it.
        """
    ...
    def _close_fragment(
        ...
        """
        Realize one short exit fragment and update all performance ledgers.
        """
    ...
    def _manage_open_position(self, state: Dict[str, object]) -> None:
        """
        Manage one short open position using the current execution bar.
...
def load_candidates(path: str) -> List[CandidateConfig]:
    """Read the short candidate CSV and convert each row into a typed candidate object."""
    if not os.path.exists(path):
    ...
    if not any(column in df.columns for column in ["combined_rank", "rank", "backtrader_rank"]):
...
def run_one_candidate(candidate: CandidateConfig, catalog: MarketDataCatalog, prefer_intraday: bool) -> Dict[str, object]:
    """
    Run one short candidate in its own Cerebro instance and return a flat result row.
    """
    ...
    cerebro.addstrategy(MavenShortCandidateStrategy, candidate=candidate, feed_map=feed_map)
...
def write_summary(
    ...
    """Write a markdown summary with the key short-side run stats and top survivors."""
    ...
    with open(summary_path, "w", encoding="utf-8") as handle:
        handle.write("# Backtrader short validation summary\n\n")
        ...
        handle.write("- **strategy_side**: short-only\n")
        if has_confirmation:
...
def evaluate_candidates(
    ...
    """
    Evaluate every short candidate and write incremental results as we go.
    """
    ...
    progress = tqdm(candidates, desc="Backtrader short validation", unit="candidate")
...
def parse_args() -> argparse.Namespace:
    """Define the CLI surface for full runs and smaller smoke checks."""
    parser = argparse.ArgumentParser(description="Chronological Backtrader validation for the TRADE_ORACLE short shortlist.")
    parser.add_argument("--candidates", default=DEFAULT_CANDIDATES_PATH, help="Path to the short candidate CSV.")
    ...
    parser.add_argument(
        ...
        help="Prefix for output files. Example: results/backtrader_short",
    )
    parser.add_argument("--limit", type=int, default=None, help="Optional cap for smoke tests.")
    parser.add_argument("--start-rank", type=int, default=None, help="Optional inclusive rank lower bound.")
    parser.add_argument("--end-rank", type=int, default=None, help="Optional inclusive rank upper bound.")
    parser.add_argument(
        "--no-intraday",
        action="store_true",
...
def main() -> None:
    """Main entry point used by `python scripts/backtrader_evaluate_short.py`."""
    args = parse_args()
