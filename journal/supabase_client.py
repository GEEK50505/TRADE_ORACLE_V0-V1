"""Supabase-backed persistent state memory for TRADE_ORACLE."""

from ai.trade_oracle_supabase import build_trade_oracle_supabase_client
...
class StateManager:
    ...
    def __init__(self):
        ...
        """
        self.supabase = build_trade_oracle_supabase_client()
        self.account_state_table = "account_state"
    ...
    def update_high_watermark(self, current_equity: float) -> float:
        ...
        response = self.supabase.table(self.account_state_table).select('high_watermark').eq('id', 1).execute()
        ...
        if not response.data:
            ...
            insert_resp = self.supabase.table(self.account_state_table).insert({'id': 1, 'high_watermark': current_equity}).execute()
        ...
        if current_equity > saved_watermark:
            ...
            update_resp = self.supabase.table(self.account_state_table).update({'high_watermark': current_equity}).eq('id', 1).execute()
            # verbose logging for debugging: show response metadata
            try:
